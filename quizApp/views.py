...
@user_passes_test(lambda u: u.groups.filter(name='quiz_makers').count() == 1, login_url='/accounts/login/')
def index(request):
...
@login_required(login_url='/accounts/login/')
@user_passes_test(lambda u: u.groups.filter(name='quiz_makers' or 'quiz_takers').count() == 1, login_url='/accounts/login/')
